...
def get_city_sales(city_name):
    ...
    url = "https://www.dongchedi.com/motor/pc/car/rank_data?new_energy_type=&month=2022{0}&count=1000&city_name={1}&rank_data_type=64".format(last_month,city_name)
    ...
    if num == 0:
        ...
        for i in range(0,len(html_body['data']['list'])):
            ...
            results = [city_name,series_id,series_name,min_price,max_price,rank,count,car_review_count,
...
out_file = "懂车帝_22{0}分城市销量_全部（交强险）.csv".format(last_month)
...
for city_name in city_list:
    if city_name not in citys_finish:
        city_sales_list = get_city_sales(city_name)
...
names = ['城市名','车系id','车系名称','最低价','最高价','排名','本地销量','评论数',
