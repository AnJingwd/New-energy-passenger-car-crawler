...
def get_city_sales(file):
    base_url = 'https://price.auto.sina.com.cn/api/PaihangbangSales/getBrandSales?'
        ...
        for month in range(1,13):
            for page in range(1,10):
                ...
                for i in range(0,len(data)):
                    data_info = data[i]
                    big_brand_id = data_info['big_brand_id']             ## 品牌id
                    zhName = data_info['zhName']                         ## 品牌名称

                    sub_data_info = data_info['big_sales_volume_sub']
                    sub_brand_id = sub_data_info['sub_brand_id']         ## 子品牌id
                    sub_brand_name = sub_data_info['sub_brand_name']     ## 子品牌id
                    sales_volume = sub_data_info['sales_volume']
                    results = [year,month,big_brand_id,zhName,sub_brand_id,sub_brand_name,sales_volume,page,d2]
                    write_csv(results, file)
            print('第',year,' 年 ',month,' 月 数据','抓取完成!')

out_file = "新浪汽车_品牌_月度销量_{0}（交强险）.csv".format(d1)
names = ['年','月份','品牌id','品牌名称','子品牌id','子品牌名称','销量','页码','dt']
